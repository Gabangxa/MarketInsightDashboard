...
import { TrendingUp, AlertTriangle, Clock, Loader2 } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function TrendWidget({ exchanges = ["bybit"] }: Props) {
  const { symbol, timeframe, setTimeframe, period, setPeriod, historicalData, isLoading, error, indicators } =
    useIndicatorData(exchanges);

  const trendIndicators = TREND_IDS.map((id) => ({ id, result: indicators[id] })).filter(
    (x) => x.result
  );

  return (
    <Card className="h-full p-4 flex flex-col overflow-hidden" data-testid="widget-trend">
      {/* Header */}
      ...
      {/* Content */}
      <div className="flex-1 overflow-auto space-y-3 min-h-0" data-testid="trend-indicators">
        {isLoading ? (
