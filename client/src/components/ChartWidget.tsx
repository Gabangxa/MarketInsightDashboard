...
export default function ChartWidget({
  ...
  const { combinedData, loadHistory, fetchingRef, earliestTimestampRef } =
    useChartData({ symbol, timeframe, realTimeDataMap: priceData });
  ...
  useEffect(() => {
    ...
    if (chartType === "candlestick") {
      ...
      seriesRef.current = chart.addSeries(LineSeries, { color: "#8B5CF6", lineWidth: 2 });
    }
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="icon" onClick={onConfigure} data-testid="button-chart-configure">
